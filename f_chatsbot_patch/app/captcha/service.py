...
#
# Кик делается через ban_chat_member() сразу с unban_chat_member() следом —
# это стандартный паттерн Bot API для "убрать из группы без бана": человек
# может вступить обратно немедленно, без ожидания и без пугающего
# "вас заблокировал администратор". Отдельная пауза перед разрешением
# повторного входа не нужна: сам факт кика возможен только после того, как
# истекут все CAPTCHA_TIMEOUT секунд капчи — это уже ограничивает скорость
# цикла "вступил → не прошёл → кикнут" примерно раз в минуту на аккаунт,
# больше добавлять нечего, а для живого человека, который просто не успел
# кликнуть, каждая лишняя секунда ожидания — это шанс, что он просто уйдёт
# и забудет про группу.
...
class CaptchaService:
    ...
    async def _timer_task(self, captcha_id: str, timeout: int):
        try:
            ...
            # и может вступить и попробовать снова СРАЗУ ЖЕ — см. комментарий у
            # константы выше про ban+unban вместо ban с отложенным until_date.
            try:
